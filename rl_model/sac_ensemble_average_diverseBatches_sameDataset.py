...
from rl_model.utils import soft_update, hard_update
from rl_model.model_simple import GaussianPolicy, QNetwork
import numpy as np
import collections
...
class SAC(object):
    ...
    def update_critic(self,
        ...
        with torch.no_grad():
            qf1_next_target, qf2_next_target = self.critic_target[critic_index](next_state_batch, next_state_action)
            min_qf_next_target = torch.min(qf1_next_target, qf2_next_target) - self.alpha * next_state_log_pi
            next_q_value = reward_batch + mask_batch * self.gamma * min_qf_next_target.squeeze()

        qf1, qf2 = self.critic[critic_index](state_batch, action_batch)
        # Two Q-functions to mitigate positive bias in the policy improvement step
        qf1_loss = F.mse_loss(qf1.squeeze(), next_q_value)  # JQ = 𝔼(st,at)~D[0.5(Q1(st,at) - r(st,at) - γ(𝔼st+1~p[V(st+1)]))^2]
        qf2_loss = F.mse_loss(qf2.squeeze(), next_q_value)  # JQ = 𝔼(st,at)~D[0.5(Q1(st,at) - r(st,at) - γ(𝔼st+1~p[V(st+1)]))^2]

        qf_loss = qf1_loss + qf2_loss

        self.critic_optim[critic_index].zero_grad()
        qf_loss.backward()
        self.critic_optim[critic_index].step()

        qf1_loss_values[critic_index] = qf1_loss.detach()
        qf2_loss_values[critic_index] = qf2_loss.detach()
    ...
    def update_parameters_sac_offline(self,
        ...
        for epoch in range(num_epochs):
            for i, (state_batch, action_batch, next_state_batch, reward_batch, shard_batch) in enumerate(tqdm(memory, desc= "Train")):
                ...
                qf1_loss_value = torch.zeros(len(self.critic))
                ...
                for critic_index in range(len(self.critic)):
                    ...
                    temp_next_state_batch = next_state_batch[temp_filter]
                    temp_mask_batch = mask_batch[temp_filter]

                    temp_qf1_loss_value, temp_qf2_loss_value = self.update_critic(temp_state_batch,
                                                                        temp_action_batch,
                                                                        temp_reward_batch,
                                                                        temp_next_state_batch,
                                                                        temp_mask_batch,
                                                                        critic_index)
                    qf1_loss_value[critic_index] = temp_qf1_loss_value
                    qf2_loss_value[critic_index] = temp_qf2_loss_value

                qf1_loss_value = torch.mean(qf1_loss_value)
                qf2_loss_value = torch.mean(qf2_loss_value)

                log_pi, policy_loss_value = self.update_actor(state_batch)

                alpha_loss_value, alpha_tlogs_value = self.update_alpha(log_pi)

                for j in range(self.size_ensemble):
                    soft_update(self.critic_target[j], self.critic[j], self.tau)
            ...
            if not os.path.exists(self.config.env_rl['save_path'] + '/output/plots/'):
                os.makedirs(self.config.env_rl['save_path'] + '/output/plots/')

            plt.savefig(self.config.env_rl['save_path'] + "/output/plots/critics_diff_{}.png".format(env_name))
            plt.show()
    ...
    def save_model(self,
        ...
        :return:
        """
        if not os.path.exists(self.config.env_rl['save_path'] + '/output/models/'):
            os.makedirs(self.config.env_rl['save_path'] + '/output/models/')

        if actor_path is None:
            actor_path = self.config.env_rl['save_path'] + "/output/models/sac_actor_{}".format(env_name)
        if critic_path is None:
            critic_path = self.config.env_rl['save_path'] + "/output/models/sac_critic_{}".format(env_name)
